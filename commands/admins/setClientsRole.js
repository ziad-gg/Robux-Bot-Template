...
async function GlobalExecute(message, interaction, global) {
  ...
  const role = controller.guild.roles.cache.get(interaction ? interaction[0].id : controller[0]);

  console.log(role);
  
  if (!role || role.name === '@everyone' || role.name === '@here') return controller.replyNoMention({ content: '❌ **قم بتحديد رتبة صالحة!**' });
  ...
  interaction.replyNoMention({ content: '✅ **تم بنجاح تحديد رتبة العملاء!**' });
};